...
internal static class Interop
{
	...
	// Token: 0x06000002 RID: 2 RVA: 0x000021F9 File Offset: 0x000003F9
	...
	// Token: 0x06000003 RID: 3 RVA: 0x0000220E File Offset: 0x0000040E
	...
	internal struct ErrorInfo
	{
		...
		// (get) Token: 0x06000006 RID: 6 RVA: 0x0000223C File Offset: 0x0000043C
		...
		// (get) Token: 0x06000007 RID: 7 RVA: 0x00002244 File Offset: 0x00000444
		...
		// Token: 0x06000009 RID: 9 RVA: 0x00002282 File Offset: 0x00000482
	...
	{
		// Token: 0x0600000A RID: 10 RVA: 0x000022AA File Offset: 0x000004AA
		...
		{
			byte* pointer = stackalloc byte[(UIntPtr)256];
			ValueUtf8Converter valueUtf8Converter = new ValueUtf8Converter(new Span<byte>((void*)pointer, 256));
			int result = Interop.Sys.Stat(MemoryMarshal.GetReference<byte>(valueUtf8Converter.ConvertAndTerminateString(path)), out output);
		...
		internal unsafe static int LStat(ReadOnlySpan<char> path, out Interop.Sys.FileStatus output)
		{
			byte* pointer = stackalloc byte[(UIntPtr)256];
			ValueUtf8Converter valueUtf8Converter = new ValueUtf8Converter(new Span<byte>((void*)pointer, 256));
			int result = Interop.Sys.LStat(MemoryMarshal.GetReference<byte>(valueUtf8Converter.ConvertAndTerminateString(path)), out output);
			valueUtf8Converter.Dispose();
			return result;
		}
		...
		// Token: 0x0600001D RID: 29 RVA: 0x00002374 File Offset: 0x00000574
		...
		internal struct DirectoryEntry
		{
			...
			{
				ReadOnlySpan<byte> bytes = (this.NameLength == -1) ? new ReadOnlySpan<byte>((void*)this.Name, new ReadOnlySpan<byte>((void*)this.Name, 256).IndexOf(0)) : new ReadOnlySpan<byte>((void*)this.Name, this.NameLength);
